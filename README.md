...
- 가시·추진·요새화·파열을 포함한 상태 12종과 카드·상태·지형 키워드 설명
- 3턴 쿨타임 이동 궤적 기술과 처치 초기화·세 갈래 공격 연출
- 단계마다 둘 중 하나만 고르는 캐릭터별 기술 형태·모듈·궁극기 분기
...
카드 클릭/숫자 1~8로 조립하고 Enter로 방출합니다. Space는 응축, 방향키는 이동, Backspace는 이동 취소, 우클릭은 파편 취소입니다. K는 가리킨 적 방향의 쿨타임 기술, G는 이번 원정의 배타적 분기 성장 화면입니다. 상세 설명은 카드 호버와 사전, 전체 조작은 게임의 `?`와 [실행 문서](PROTOTYPE.md)에서 확인할 수 있습니다.
...
저장 위치와 복구 방식은 [기본 기능 v9](Docs/BASICS_V9.md), 새 전장과 규칙 버전 10은 [전장·카드 UI 개편](Docs/BATTLEFIELD_V10.md), 유물·두 성장 축은 [전투와 성장 v12](Docs/COMBAT_GROWTH_V12.md), 최신 전투 규칙 13은 [전투 흐름과 분기 성장 v13](Docs/COMBAT_FLOW_V13.md)에 정리했습니다.

## 개발과 검증

구조는 `Runtime/Presentation → Application → Core`입니다. Unity에서 Test Runner의 EditMode 테스트를 실행할 수 있습니다. 최신 검증은 **148개 테스트 통과**, Full HD/720p 화면 fixture 30장, 게임 Runtime 오류 0건입니다. 플레이어 기준점, 새 궤적, 쿨타임 기술, 배타 분기와 저장·전투 회귀를 함께 확인했습니다. 이는 Windows 빌드 수동 플레이, 전체 런 밸런스 검증이나 성능 측정 결과를 의미하지 않습니다.
