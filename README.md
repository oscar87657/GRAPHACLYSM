...
저장 위치와 복구 방식, 검증 방법은 [기본 기능 v9](Docs/BASICS_V9.md)에 정리했습니다.

## 개발과 검증

구조는 `Runtime/Presentation → Application → Core`입니다. Unity에서 Test Runner의 EditMode 테스트를 실행할 수 있습니다. v9 검증은 **137개 테스트 통과**, Full HD/720p 화면 fixture 22장, Runtime 오류 0건입니다. 전투 중 저장, 보상/방/2층 전이, 백업 복구, 저장 실패 후 재시도, 작도 중 일시정지와 설정 재실행 유지를 확인했습니다. 이는 전체 런 밸런스 검증이나 성능 측정 결과를 의미하지 않습니다.
