...
- 방 완료 경험치로 해금하는 캐릭터별 원정 성장과 전투 기술·궁극기 변형
...
카드 클릭/숫자 1~8로 조립하고 Enter로 방출합니다. Space는 응축, 방향키는 이동, Backspace는 이동 취소, 우클릭은 파편 취소입니다. K는 전투당 한 번인 전투 기술, G는 이번 원정 성장 화면입니다. 상세 설명은 카드 호버와 사전, 전체 조작은 게임의 `?`와 [실행 문서](PROTOTYPE.md)에서 확인할 수 있습니다.
...
저장 위치와 복구 방식은 [기본 기능 v9](Docs/BASICS_V9.md), 새 전장과 규칙 버전 10은 [전장·카드 UI 개편](Docs/BATTLEFIELD_V10.md), 전투·성장 규칙 버전 12는 [전투와 성장 v12](Docs/COMBAT_GROWTH_V12.md)에 정리했습니다.

## 개발과 검증

구조는 `Runtime/Presentation → Application → Core`입니다. Unity에서 Test Runner의 EditMode 테스트를 실행할 수 있습니다. 최신 검증은 **146개 테스트 통과**, Full HD/720p 화면 fixture 29장, Runtime 오류 0건입니다. 새 상태 순서, 두 성장 저장, 전투 기술, 유물 발동·그림과 기존 저장·전투 회귀를 함께 확인했습니다. 이는 Windows 빌드 수동 플레이, 전체 런 밸런스 검증이나 성능 측정 결과를 의미하지 않습니다.
