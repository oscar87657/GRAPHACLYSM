...
기록 이안 조준 안내: 식의 기준점·기록 거리 원·전장 안쪽 경계를 표시한다. 불가능한 위치도 붉은 도안과 `기록 불가` 안내를 유지한다. 전장 밖에서는 가장자리에서 미리보기를 유지하며 확정할 수 없다. 실제 거리·비용·저장 규칙은 변경하지 않는다. 미리보기는 기존 전투 소유 버퍼를 재사용한다.

배포 화면 해상도: 새 환경은 모니터 해상도를 자동으로 사용한다. 기존 설정은 보존하므로 화면이 흐릿하면 설정에서 `자동 · 모니터 해상도`를 선택하고 적용한다. 자동 창 모드는 바탕화면 크기의 85%를 사용한다. 원정·잔광 저장 규칙은 변경하지 않는다.

수식 파편 카드를 순서대로 조립하여 그래프를 만들고, 같은 작도로 적을 공격하거나 자신을 강화하는 Unity 덱 빌딩 로그라이트 프로토타입입니다.

## 앞으로의 개발 순서

**UI·편의성·미감 정리 → 스토리 완성·게임 연동·용어 재편 → 무한 등반·레벨 디자인·밸런스 개편**

1. **UI 전면 정리 — 다음 작업:** 불필요한 문구와 개발 흔적을 걷어내고, 필요한 곳의 작은 `?` 도움말, 버튼 삭제·통합·재배치, 화면 구성과 조작의 일관성을 다듬습니다.
2. **스토리와 용어 정립 — 후속 작업:** 전체 이야기를 먼저 완성한 뒤 층·보스·이벤트·대사에 연결하고 고유명사와 설명을 정리합니다. **히든 엔딩과 클리어 보상으로 캐릭터별 세 번째 전투 스타일을 해금하는 방안은 검토 중**이며, 조건과 능력은 아직 확정하지 않았습니다.
3. **무한 등반 모드 — 후속 작업:** 끝없이 오르는 콘텐츠를 설계하면서 적·방·보스의 배치, 성장·보상 흐름, 카드·유물 효과와 장기 등반 밸런스를 함께 개편합니다.

세 단계는 앞으로 할 일이며 완료된 기능 목록이 아닙니다. [상세 로드맵과 단계별 완료 기준](Docs/ROADMAP.md) · [현재 구현 상태와 검증 범위](Docs/PROJECT_CHECKPOINT_V42.md)

## 현재 구현

연속 탑 지도 UI v42까지의 상태입니다. 원정 저장 규칙은 41, 계정 저장 버전은 4입니다. 이전 저장은 당시 규칙에 따라 복원되므로 새 원정과 구성이 다를 수 있습니다.

- **전투:** 이안의 집행·기록, 루나의 조율·관측과 각 방식의 궁극기. 작도·이동·자가 적중·응축을 결합합니다.
- **원정:** 보스 후보를 추첨하는 5층 원정. 입구부터 정상까지 연결된 지도를 드래그·휠로 둘러보며 탑 배경도 함께 움직입니다.
- **원정 성장:** 전투 방식별 16노드 트리. 시작 3점에 진행 중 최대 6점을 얻어 총 9점 안에서 투자합니다.
- **보상과 상점:** 원하는 것만 받는 가로 카드형 전리품, 색별 파편 선택, 은화·유물·열쇠·봉인 상자와 상점 상품.
- **영구 성장:** 잔광 24노드 / 36단계. 분대 개방, 유물 지참, 다음 원정을 위한 보조 효과.
- **안내와 저장:** 클릭하는 키워드 설명, 카드·적 사전, 조작 안내, 자동 저장·이어하기·일시정지·설정.

전체 스토리 연동, 히든 엔딩, 캐릭터별 세 번째 스타일, 무한 등반은 아직 구현된 것으로 취급하지 않습니다. 기존 5페이지 조작 안내는 앞으로 개편할 튜토리얼·상황별 도움말과 구분합니다.
...
2. Unity Hub에서 프로젝트를 추가하고 **Unity 6000.3.23f1**로 엽니다.
3. 패키지 설치와 에셋 임포트가 끝나면 `Assets/Scenes/SampleScene.unity`를 열고 Play합니다.
4. 새 원정에서 캐릭터와 전투 방식을 선택하거나 저장된 원정을 이어 합니다.

카드 클릭/숫자키로 조립하고 Enter로 방출, Space로 응축합니다. 기술은 화면의 버튼과 조준 안내를 따릅니다. G는 원정 성장, Esc는 일시정지, F1은 조작 안내입니다. 지도는 드래그·휠로 이동하며 현재 위치·입구·꼭대기로 돌아가는 버튼을 제공합니다. 세부 조작과 실행 기록은 [실행 문서](PROTOTYPE.md)를 참고하세요.

## 개발과 검증

구조는 `Runtime/Presentation → Application → Core`입니다. Unity Test Runner에서 EditMode 테스트를 실행할 수 있습니다.

**기존 v42 검증 기록:** EditMode 583/583 통과, 지도 화면 12장, 게임 Runtime 오류 0건. 층 경계·입구·정상, 1080p/720p, 드래그 입력, 첫 전투 후 지도 복귀와 저장 재생을 확인했습니다. 이는 [v42 작업 당시의 결과](Docs/PROJECT_CHECKPOINT_V42.md)이며 이번 문서 정리에서 게임 검사를 새로 실행했다는 뜻은 아닙니다. 전체 5층 수동 밸런스 검증과 최종 배포 빌드는 별도입니다.

`Assets`와 `.meta`, `Packages`, `ProjectSettings`, 기획 문서를 버전 관리합니다. Unity 캐시·개인 설정·빌드·`Logs`의 검증 사본과 백업은 제외합니다. 문서의 로그·상세 캡처 경로는 개발 PC의 로컬 기록일 수 있습니다.

## 문서

- [앞으로의 개발 로드맵](Docs/ROADMAP.md)
- [1차 정리 · 현재 구현과 검증 범위](Docs/PROJECT_CHECKPOINT_V42.md)
- [전체 기획](GAME_DESIGN_DOCUMENT.md)
- [실행·조작·검증](PROTOTYPE.md)
- [코드 구조](ARCHITECTURE.md)
- [개발 재개용 지침](AGENTS.md)
- [아트 방향과 출처](ART_DIRECTION.md)
- [사용 글꼴 라이선스](Docs/FontLicenses)

## 이전 개발 이력

<details>
<summary>이전 README 기록 펼치기 — v16~v21 당시 설명이며 현재 사양이 아닙니다</summary>

아래는 과거의 구현·검증·미구현 설명을 보존한 기록입니다. 여기서 “최신”이나 “현재”는 작성 당시를 뜻합니다. 현재 기능은 위의 구현 요약과 v42 체크포인트, 다음 작업은 로드맵을 기준으로 확인하세요.
