...
{
    [InitializeOnLoad]
    public static class WideV8Smoke
    {
        private const string Key = "GRAPHACLYSM.WideV8Smoke";
        static WideV8Smoke()
        {
            EditorApplication.playModeStateChanged += change =>
            {
                if (change != PlayModeStateChange.EnteredPlayMode || !SessionState.GetBool(Key, false)) return;
                SessionState.SetBool(Key, false); new GameObject("Wide V8 fixture").AddComponent<WideV8Runner>();
            };
        }
    ...
    public sealed class WideV8Runner : MonoBehaviour
    {
        ...
        private void Start()
        {
            output=Path.GetFullPath("../WideV8Captures"); Directory.CreateDirectory(output);
        ...
        private void Update()
        {
            try
            {
                ...
                switch(stage++)
                {
                    ...
                    case 46: SetSize(1920,1080);Toggle("codexOpen",false);StartRun(302,0);flow.CurrentRun.TrySelectMapNode(0);InstallResearchBattle();break;
                    case 47: Capture("24-new-fragments-hand");break;
                    case 48: Play("frag.home");Play("frag.lissajous");Play("frag.shear");Play("frag.limacon");Refresh();break;
                    case 49: Capture("25-research-compound-curve");break;
                    case 50: Play("frag.phase");Play("frag.cube");Play("frag.eclipse");Play("frag.contract");Toggle("showBattleDetails",true);Refresh();break;
                    case 51: if(flow.CurrentRun.CurrentBattle.Battle.PlayedCardCount!=8)throw new InvalidOperationException("Eight-fragment weave missing");Capture("26-full-loom");break;
                    case 52: Toggle("showEquation",true);SetSize(1280,720);break;
                    case 53: Capture("27-full-loom-details-720p");break;
                    case 54:
                        File.WriteAllText(Path.Combine(output,"smoke-result.txt"),"Full HD + 720p; 27 captures\nSix researched fragments, compounded and full 8-fragment loom; equal 95 px axes / 950 x 760 field; main dictionary, filters and pages; new relics; 3 floor maps; unique card seals; shifted graph geometry and reveal; condense health cost; movement dock/undo; hover/details; 8-card hand; rooms/removal.\nEditor fixtures (including map fast-forward and 8-card refill), not full-run balance.\nRuntime errors: "+errors);
        ...
        private void InstallResearchBattle()
        {
            const BindingFlags flags=BindingFlags.Static|BindingFlags.NonPublic;
            var field=(Rect)typeof(GraphaclysmModernView).GetField("Field",flags).GetValue(null);
            var point=typeof(GraphaclysmModernView).GetMethod("FieldPoint",flags);
            var zero=(Vector2)point.Invoke(null,new object[]{0d,0d});
            var right=(Vector2)point.Invoke(null,new object[]{1d,0d});
            var up=(Vector2)point.Invoke(null,new object[]{0d,1d});
            if(field.width!=950 || field.height!=760 || Mathf.Abs((right-zero).magnitude-95)>.01f || Mathf.Abs((up-zero).magnitude-95)>.01f)
                throw new InvalidOperationException("Wide field must preserve equal axes.");
            var c=flow.CurrentCharacter;
            var b=new BattleSession(new BattleDefinition(c.MaxHealth,c.MaxEnergy,new[]{
                new EnemyDefinition("a","첫 번째 기어",6,0,200,0),new EnemyDefinition("b","두 번째 기어",3.2,1.8,200,0),new EnemyDefinition("c","세 번째 기어",5.5,-1.5,200,0)
            },c.Archetype,fragments:true),c.MaxHealth,0,0,6);
            b.TryMovePlayer(0,1.5);
            var ids=new[]{"home","lissajous","shear","limacon","phase","cube","eclipse","contract","echo","orbit","twin","petal"};
            var cards=new CardDefinition[ids.Length];for(int i=0;i<ids.Length;i++)cards[i]=FragmentCardCatalog.Find("frag."+ids[i]);
            Array.Reverse(cards);var deck=new DeckSession(cards,8,new KeepOrder(),true,8);
            typeof(RunGameSession).GetProperty("CurrentBattle").SetValue(flow.CurrentRun,new BattleGameSession(b,deck));Refresh();
        }
