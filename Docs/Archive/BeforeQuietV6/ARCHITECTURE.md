...
- 기본 조립은 `GraphaclysmModernView`. Battle/Screens/FragmentView partial이 전투·지도·파편 조립을 분리한다. CalculatorView는 현재 방/덱 정리와 legacy 계산기 메서드를 가진다. 기본 런은 계산기 화면으로 진입하지 않는다. 이전 PrototypeView/TacticalView는 명시적 legacy Editor 진단에서만 생성한다.
...
- `SkillVisual[34]` (현재 10종 + legacy 계산기 11종 + 기술 13종)은 Awake에서 구성한다. 카드별 Vector2[41] 그림, 비용·능력·상세 문자열을 카드 풀 수명 동안 재사용한다.
...
현재 검증은 EditMode 97개와 실제 화면 fixture 24장이다. GC/프레임 시간의 신규 Profiler 측정은 하지 않았으며 기존 성능 보고를 v5 측정으로 표현하지 않는다. 사용자가 열어 둔 Editor와 격리된 검증 사본을 사용한다.
